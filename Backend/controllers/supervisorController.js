...
exports.submitLocationData = async (req, res) => {
  try {
    ...
    const locIdNumber = (loc_id.replace(/\D/g, ''), 10);
    ...
    console.log("ippp",mlInput)
    // Make a request to the ML model
    const mlResponse = await axios.post('http://localhost:3000/predict', mlInput);
    console.log("hii",mlResponse)
